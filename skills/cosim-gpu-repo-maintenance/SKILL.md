...
- locate or update the `.agents`, `gem5`, `qemu`, or `gem5-resources`
  submodule pointer
...
splitting skill commits, or preparing project commits.
...
2. Classify changes by owner:
   ...
   - `.agents` skills submodule
   - `gem5` source submodule
   - `qemu` source submodule
   - `gem5-resources` data submodule
   - generated artifacts or local scratch

3. Commit inside submodules before recording their superproject gitlink.
...
6. Before final reporting, show current submodule pointers and remaining dirty
   files.
